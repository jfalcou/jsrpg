...
export function placeItemInEquip(itemData, slotId) {
    ...
    if (safeEquipContainer) {
        ...
        if (eqConfig) {
            // Placement absolu sur la grille du parent
...
export function initEquipDOM() {
    ...
    equipContainer.style.position = 'relative'; // Indispensable pour l'absolu
    equipContainer.style.display = 'grid';
    equipContainer.style.gridTemplateColumns = `repeat(${UI_CONFIG.equipGrid.width}, var(--grid-cell))`;
    equipContainer.style.gridTemplateRows = `repeat(${UI_CONFIG.equipGrid.height}, var(--grid-cell))`;
    equipContainer.style.gap = `var(--grid-gap)`;

    // On force la taille du conteneur
    equipContainer.style.width = `${UI_CONFIG.equipGrid.width * STEP}px`;
    equipContainer.style.height = `${UI_CONFIG.equipGrid.height * STEP}px`;
